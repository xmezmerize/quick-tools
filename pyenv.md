...
```python
pyenv global 3.12.6    # define versão global do sistema

pyenv local 3.12.6    # define versão no diretório do projeto

pyenv shell 3.12.6    # define versão apenas para a sessão atual
```
...
```
Aqui, devemos lembrar que é preciso definir a versão do Python antes de criar o ambiente virtual, pois, o Python do ambiente virtual usará - à princípio - uma versão de uso global.
...
```python
python -m pip install <package>    # instala pacote no Python ativo

python -m pip list    # lista pacotes instalados

python -m pip show <package>    # detalhes do pacote
